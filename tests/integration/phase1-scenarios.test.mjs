...
describe("一期轻量闭环 · 25 项核心场景集成测试", () => {
  ...
  // 4. 区域经理可以查看本区域子门店
  it("4. 区域经理可以查看本区域子门店，不可查看跨区域门店", async () => {
    ...
    assert.equal(viewStoreC.status, 404, "华东区域经理不可查看北京朝阳店")
  })
  ...
  // 8. OSS 重复对象不重复登记
  it("8. OSS 重复对象不重复登记 (object_key 幂等)", async () => {
    const key = "oss/storeA/audio-20260825-001.mp3"
    const first = await server.req("POST", "/api/audio_files", {
      object_key: key,
    ...
    const second = await server.req("POST", "/api/audio_files", {
      object_key: key,
      file_name: "audio-001.mp3",
      device_sn: "DEV-001",
      status: "PENDING",
    ...
    assert.equal(second.data.duplicate, true, "重复 object_key 必须返回 duplicate: true")
    assert.equal(second.data.item.id, first.data.id || first.data.item.id)
  })

  // 9. ASR 重复回调不重复写结果
  it("9. ASR 重复回调不重复写结果", async () => {
    ...
    const update1 = await server.req("PATCH", `/api/transcripts/${trId}`, {
      summary: "处方用药咨询",
    }, { "X-Yuqi-Service-Token": env.serviceToken })
    assert.equal(update1.status, 200)
    assert.equal(update1.data.summary, "处方用药咨询")
  })
